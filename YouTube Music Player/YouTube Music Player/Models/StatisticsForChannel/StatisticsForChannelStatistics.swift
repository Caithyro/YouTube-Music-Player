...
struct StatisticsForChannelStatistics: Codable {
    ...
    init(from decoder: Decoder) throws {
        ...
        subscriberCount = try values.decodeIfPresent(String.self,
                                                     forKey: .subscriberCount)
        hiddenSubscriberCount = try values.decodeIfPresent(Bool.self,
                                                           forKey: .hiddenSubscriberCount)
        videoCount = try values.decodeIfPresent(String.self,
                                                forKey: .videoCount)
    }
